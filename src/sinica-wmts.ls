...
L.SINICA.WMTS = L.TileLayer.WMTS.extend do
  _url: 'http://gis.sinica.edu.tw/tileserver/wmts?'
  defaultWmtsParams: do
  ...
  initialize: (options) ->
    @defaultWmtsParams.'layer' = @layer
    wmtsParams = L.extend({}, @defaultWmtsParams)
    tileSize = 256
    for i of options
      wmtsParams[i] = options[i] if (not @options[i]?) && i isnt 'matrixIds'

    wmtsParams.width = wmtsParams.height = tileSize * if L.Browser.retina then 2 else 1
    @wmtsParams = wmtsParams
    @matrixIds = @getDefaultMatrix();
...
for i of SinicaLayers
    L.SINICA.[SinicaLayers[i]] = L.SINICA.WMTS.extend do
      name: [i]
